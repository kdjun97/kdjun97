### About Me
- **I'm Dongjun Kim, Jumy.**
- **Mobile**, **IoT** 그리고 **Automation Solutions** 에 관심이 많습니다.
- [Linked-In Profile](https://www.linkedin.com/in/dongjun-kim-ab3a79255/)
<br>

### App Side Project
- **달릴까말까** (25.06 ~ 25.07 | Flutter)
  - 🐴 [달릴까말까 Github](https://github.com/APPisode-Dev/run-or-not) - 간단한 내기용 복불복 레이스 앱
- **오목완** (24.08 ~ | Swift)
  - 🟠 [오목완 Github](https://github.com/begin-a-gain/omokwan-ios) - 오목 돌을 하나씩 놓으며 하루의 목표를 완성하는 목표 관리 앱
- **Gwasuwon(과수원)** (24.05 ~ 24.08 | Swift)
  - 🧑‍🏫 [과수원 Github](https://github.com/prography-team8/gwasuwon-ios) - 과외 선생님과 학생을 매칭 및 모니터링 서비스 제공 앱
<br>

### Experience
- ☀️ Hanwha Solutions QCells (23.01 ~ | Swift, Flutter)
  - [Q.OMMAND Pro Gen4] 차세대 태양광·에너지 시스템 설치·점검을 위한 현장 엔지니어용 앱 개발 (25.06 ~ | Swift)
  - [Q.OMMAND Pro Gen3] 태양광·에너지 시스템 설치·점검을 위한 현장 엔지니어용 앱 개발 (24.04 ~ | Swift)
  - [Q.OMMAND Home 3.0] 가정용 에너지 모니터링 앱 개발 (24.03 ~ | Swift)
  - [Q.PLANET] 분산된 에너지 자원을 통합 관리하는 한국형 가상발전소(VPP) 앱 개발 (24.02 ~ 24.05 | Swift)
  - [Q.OMMAND Pro 1.0]  태양광·에너지 시스템 설치·점검을 위한 현장 엔지니어용 앱 개발 (23.05 ~ 24.04 | Flutter)
  - [Q.OMMAND Home 2.0] 가정용 에너지 모니터링 앱 개발 (23.04 ~ 24.03 | Flutter)
- 🧑‍🌾 Nuru(느루) (22.07 ~ 22.12)
  - 시뮬레이션 게임 컨셉의 힐링 앱, **홀로서기** 개발 (22.09 ~ 22.12 | Flutter)
  - 스마트 임산물 관리 앱 Nuru, IoT 연동 기능 개발 (22.07 ~ 22.08 | Kotlin)
- **Internships**
  - 💪 피엔케이솔루션 (22.03 ~ 22.06)
    - 헬스장 공간·기구 운영 관리 앱  ‘커넥티드짐 공간 관리 앱’ 개발 (Flutter)
    - 기구 상태·운동 데이터 모니터링 ‘커넥티드짐 HMI 앱’ 개발 (Flutter)
    - 운동 데이터 기록 및 개인화 관리 앱 ‘커넥티드짐 사용자 앱’ 개발 (Flutter)
  - 🦠 주식회사홀잡펠이펙티브마이크로브스(HEM) (21.06 ~ 21.08)
    - AutoML Image Classification 학습을 위한 이미지 라벨링 웹 도구 개발 (21.07 ~ 21.08 | Flutter)
    - 건강 검진 데이터 Text Recognition 기능 개발 (21.06 ~ 21.07 | Flutter)
<br>

### Activity
- IT 연합 동아리 [Prography](https://prography.org/) 9기 (24.05 ~ 24.08)  
<br>

### Education
- **🎓 Handong Global Univ. (2023)**
  - Computer Science & Electronic Engineering (double major)
<br>
